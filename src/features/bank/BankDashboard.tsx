...
import { Wallet, Clock, CheckCircle, XCircle, Loader2, Calendar, Filter, RefreshCw } from 'lucide-react';
...
export const BankDashboard = () => {
    ...
    const handleResetFilters = () => {
        const emptyFilters = { start_date: '', end_date: '' };
    ...
    // Fonction pour afficher la période active en texte
    const getActivePeriodText = () => {
        if (filters.start_date && filters.end_date) {
            return `Du ${new Date(filters.start_date).toLocaleDateString('fr-FR')} au ${new Date(filters.end_date).toLocaleDateString('fr-FR')}`;
        }
        return "Sur les 7 derniers jours"; // Valeur par défaut de votre backend
    };
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-300 p-2 md:p-6">
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <form onSubmit={handleFilterSubmit} className="flex flex-col md:flex-row items-end gap-4">
                    ...
                    <div className="w-full md:flex-1">
                        <label className="block text-xs font-medium text-slate-700 mb-1">Filtrer par période</label>
                    ...
                    <div className="w-full md:w-auto flex gap-2">
                        <button 
                            ...
                            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors"
                        >
                            <RefreshCw className="h-4 w-4" /> <span className="hidden sm:inline">Réinitialiser</span>
                        </button>
                        <button 
                            type="submit" 
                            disabled={isLoading}
                            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors disabled:opacity-70"
                        >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="rounded-xl border border-slate-100 bg-white p-6 shadow-sm lg:col-span-2">
                            ...
                            <p className="text-xs text-slate-500 mb-6 flex items-center gap-1">
                                <Calendar size={12} /> {getActivePeriodText()}
                            </p>
