...
import { Search, Building2, Download, Eye, Loader2, FileText, Smartphone } from 'lucide-react';
...
export const CompanyDeclarations = () => {
    const { 
        ...
        downloadReceipt // <-- Importation de la fonction de téléchargement
    } = useCompanyDashboard();

    // État local pour gérer le chargement spécifique d'une quittance
    ...
    // Gestionnaire de téléchargement avec feedback visuel
    const handleDownloadReceipt = async (id: number, reference: string) => {
        setDownloadingId(id);
        await downloadReceipt(id, reference);
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="flex flex-wrap items-center gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                {/* Recherche par Référence Mobile Money (NOUVEAU) */}
                <div className="relative min-w-[200px] flex-1">
                    ...
                    <table className="w-full text-left text-sm text-slate-600">
                        <thead className="bg-slate-50 text-xs font-semibold uppercase text-slate-500 border-b border-slate-100">
                            <tr>
                                ...
                                <th className="px-6 py-4">Référence(s)</th>
                                <th className="px-6 py-4">Opérateur / Banque</th>
                            ...
                            {isLoading ? (
                                ...
                                declarations.map((dec) => (
                                    <tr key={dec.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        {/* Affichage intelligent des références */}
                                        <td className="px-6 py-4 font-mono text-slate-600">
                                            {dec.reference && <div>{dec.reference}</div>}
                                            {dec.mobile_reference && (
                                                <div className="text-xs text-slate-400 mt-0.5">
                                                    MoMo: {dec.mobile_reference}
                                                </div>
                                            )}
                                            {!dec.reference && !dec.mobile_reference && '-'}
                                        </td>

                                        {/* Affichage intelligent de la banque / opérateur */}
                                        <td className="px-6 py-4">
                                            <div className="font-medium text-slate-900">
                                                {['mobile_money', 'orange_money'].includes(dec.payment_mode || '') 
                                                    ? 'Opérateur Mobile' 
                                                    : (dec.bank?.bank_name || '-')}
                                            </div>
                                        ...
                                        <td className="px-6 py-4 text-right whitespace-nowrap space-x-2">
                                            ...
                                            {/* Bouton de téléchargement de la quittance */}
                                            {dec.status === 'cnps_validated' && dec.receipt_path ? (
                                                <button 
                                                    onClick={() => handleDownloadReceipt(dec.id, dec.reference || dec.mobile_reference || String(dec.id))}
                                                    disabled={downloadingId === dec.id}
                                                    className="inline-flex items-center gap-1.5 rounded-md bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 transition-colors hover:bg-emerald-100 disabled:opacity-50"
                                                    title="Télécharger la quittance officielle"
                                                >
                                                    {downloadingId === dec.id ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />} 
                                                    Quittance
                                                </button>
                                            ) : (
                                                <button 
                                                    disabled
                                                    className="inline-flex items-center gap-1.5 rounded-md bg-slate-50 border border-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-400 transition-colors opacity-50 cursor-not-allowed"
                                                    title="En attente de la quittance CNPS"
                                                >
